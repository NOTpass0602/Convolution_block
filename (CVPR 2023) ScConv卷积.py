...
class GroupBatchnorm2d(nn.Module):
    def __init__(self, c_num: int,
        ...
        assert c_num >= group_num #通道数大于等于分组数
...
#通道重构单元（CRU）：采用分割-转换-融合策略来减少通道冗余
class CRU(nn.Module):
    '''
    alpha: 0<alpha<1
    '''
...
if __name__ == '__main__':
    ...
    model = ScConv(32)
    print(model(x).shape)#torch.Size([3, 32, 64, 64])
